...
void MainTask(void) 
{
	...
	GUI_SetFont(&GUI_Fontyahei48);
	GUI_DispStringAt("CELL信号发生器", (LCD_GetXSize() - 6*48)/2 , (LCD_GetYSize() - 48)/2);
  ...
  if(File_Init())
	...
	else
	{
		...
		GUI_DispStringAt("没有找到U盘或SD卡！", (LCD_GetXSize() - 8*36)/2 , LCD_GetYSize()/2 + 36 + 10);
		cfg_flag = 1;
