...
extern GUI_CONST_STORAGE GUI_FONT GUI_Fontkai18;
...
/*		{
			...
			{
				project_flag=0;
				DROPDOWN_SetSel(hItem, project_number);
			}
			hItem = WM_GetDialogItem(pMsg->hWin, ID_TEXT_7);
			strcpy(buf,(((BL_FPGA1*)(&Rnand))->Project_Name ));
			if(strcmp(buf,"")==0)
				TEXT_SetText(hItem, "No Project");
			else
				TEXT_SetText(hItem, buf);
			hItem = WM_GetDialogItem(pMsg->hWin, ID_TEXT_8);
			sprintf(buf, "%s--%d", Rnand.fpic[picture_num].FPIC_NAMEE,picture_num);	
			strcpy(buf,Rnand.fpic[picture_num].FPIC_NAMEE);
			if(strcmp(buf,"")==0)
				sprintf(buf, "Play End");
			else
			{				
				strcat(buf,"--");
				sprintf(buf1, "%d", picture_num);					
				strcat(buf,buf1);
			}  
			TEXT_SetText(hItem, buf);
			hItem = WM_GetDialogItem(pMsg->hWin, ID_TEXT_9);
			if(mode_flag)
				TEXT_SetText(hItem, "自动");
			else
				TEXT_SetText(hItem, "手动");
...
//		TEXT_SetFont(hItem, &XBF_Font24);
	...
	HEADER_SetFont(LISTVIEW_GetHeader(hItem), &GUI_Fontkai18);
