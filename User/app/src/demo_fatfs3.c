...
extern DIR DirInf_sd;

/* 仅允许本文件内调用的函数声明 */
//static int ViewRootDir(char *_ucVolume,char buf[][16]);
...
//			{
				sprintf(path, "%s/%s", FS_VOLUME_USB,cfgname[0]);
...
//	sprintf(path, "%s/", FS_VOLUME_SD);
//	result = f_opendir(&DirInf_sd, path); /* 如果不带参数，则从当前目录开始 */
//	if (result != FR_OK)
//	{
//		printf("打开根目录失败  (%s)\r\n", FR_Table[result]);
//		return;
