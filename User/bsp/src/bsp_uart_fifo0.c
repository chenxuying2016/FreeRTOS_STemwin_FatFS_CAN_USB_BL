...
uint8_t Rnand[2048]={0};
uint8_t FPGA1_STV1[2048]={0};   //s1tv-s5tv
uint8_t FPGA1_STV2[2048]={0};   //s6tv-s10tv
uint8_t FPGA1_STV3[2048]={0};   //s11tv-s15tv
uint8_t FPGA1_RGBN[2048]={0};   //TRGB1-15
uint8_t jiexiwan_flag=0;
uint8_t  Pro_number=0;
uint8_t FPGA1_BL[2048]={0};
extern uint8_t PIC_normal_abmormal;
...
static uint8_t uart_jiexi(void);
static void uart_command_running(void);
...
void DemoCOM(void)
{
	...
	while (1)
	{	
//		if(uart_jiexi())             //uart 解析
//			uart_command_running();   // 根据串口指令的处理
		for(i=0;i<255;i++)
			for(j=0;j<255;j++)
				jiexiwan_flag = 1;
		vTaskDelay(10);
...
static uint8_t uart_jiexi(void)
{
	...
	aRxBuffer[0] = get_char();
	while(aRxBuffer[0] != 0xc9)
		aRxBuffer[0] = get_char();
	for(i=1;i<7;i++)
		aRxBuffer[i] = get_char();	
	
	while(1)
	{
		if((aRxBuffer[0]==0xc9)&&(aRxBuffer[1]==0xDB)&&(aRxBuffer[5]==0xE8)&&(aRxBuffer[6]==0xB2))   //判帧头 、帧尾
		{
			for(i=2;i<4;i++)
			sum+=aRxBuffer[i];
			if((uint8_t)(sum&0x000000FF)==aRxBuffer[4])    //和校验对
			{
				sum=0;
				if(aRxBuffer[2]==0x01)
				{
					project_number=aRxBuffer[3]; 
				}
				else if(aRxBuffer[2]==0x02)
				{
					BL_I=aRxBuffer[3]; 
				}
				else if(aRxBuffer[2]==0x03)
				{
					FPGA_U=aRxBuffer[3]; 
				}
				else if(aRxBuffer[2]==0x04)
				{
					picture_num=aRxBuffer[3]; 
				}
				else if(aRxBuffer[2]==0x05)
				{
					DAta[0]=0;                                                                                                                                                                              
					CAN_SendMessage(0x0104ffff,DAta);                                       //Reset BL
					CAN_SendMessage(0x0209ffff,DAta);                                       //Reset FPGA1                                                        //reset BL \FPGA 
				}
				else if(aRxBuffer[2]==0x06)
				{
					display_U_flag=1;                                                         //display fpga U VGH
				}
		...
		{
			for(i=0;i<6;i++)
				aRxBuffer[i] = aRxBuffer[i+1];
			aRxBuffer[6] = get_char();
    }
	}
}
...
static void uart_command_running(void)
{
  ...
  uint8_t uart[7]={0};
	...
	sprintf(filename, "Project%02d_BL_fpga1.txt", project_number);
	ReadFileData(FS_VOLUME_SD,filename,Rnand);	
	sprintf(filename, "Project%02d_fpga1_STV1.txt", project_number);
	ReadFileData(FS_VOLUME_SD,filename,FPGA1_STV1);	
	sprintf(filename, "Project%02d_fpga1_STV2.txt", project_number);
	ReadFileData(FS_VOLUME_SD,filename,FPGA1_STV2);	
	sprintf(filename, "Project%02d_fpga1_STV3.txt", project_number);
	ReadFileData(FS_VOLUME_SD,filename,FPGA1_STV3);	
	sprintf(filename, "Project%02d_fpga1_rgb_n.txt", project_number);
	ReadFileData(FS_VOLUME_SD,filename,FPGA1_RGBN);
	
	ARM_FPGA1();
	Pro_number=((BL_FPGA1*)(&Rnand))->Project_Num;
	uart[0]=0xc9;
	uart[1]=0xdb;
	uart[2]=0x02;
	uart[3]=0x01;
	...
	uart[6]=0xb2;
	for(i=0;i<8;i++)
		put_char(uart[i]);		//告诉上位机进工程正常

	if(BL_I==0xaa)
	{
		BL_I=0;
		data[0]=0x01;
		CAN_SendMessage(0x0103ffff,data);          //BL output enable vTaskDelay(20);
...
//		compare6();
	...
	else if(BL_I==0xbb)
	{
		BL_I=0;
		data[0]=0;
		CAN_SendMessage(0x0103ffff,data);          //BL output disable
	}
	...
	if(PIC_normal_abmormal==0x01)          //normal
	{
		PIC_normal_abmormal=0;
		uart[0]=0xc9;
		uart[1]=0xdb;
		uart[2]=0x05;
		uart[3]=0x01;
	...
	else 	if(PIC_normal_abmormal==0x02)   //abnormal
	{
		PIC_normal_abmormal=0;
		uart[0]=0xc9;
		uart[1]=0xdb;
